    ...
    if ( Test-BuildConfiguration )
    {
        if ( $pscmdlet.shouldprocess("Configuration module $($script:DscBuildParameters.ConfigurationModuleName) and configuration $($script:DscBuildParameters.ConfigurationName)") ) {
            ...
            try
            {
                ...
                Write-Verbose "`tRunning $($script:DscBuildParameters.ConfigurationName)"
                "Running:>_ $($script:DscBuildParameters.ConfigurationName) -outputpath $($script:DscBuildParameters.ConfigurationOutputPath) -ConfigurationData `$script:DscBuildParameters.ConfigurationData -verbose" | Write-Verbose
                
                $null = .$script:DscBuildParameters.ConfigurationName -outputpath $script:DscBuildParameters.ConfigurationOutputPath -ConfigurationData $script:DscBuildParameters.ConfigurationData -verbose    
